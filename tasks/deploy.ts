...
// that factory.
const deployContract = async (hre: HardhatRuntimeEnvironment, networkName: string) => {
	...
	const zetaTokenConsumerUniV2 = getAddress('zetaTokenConsumerUniV2', networkName as any)
	const zetaTokenConsumerUniV3 = getAddress('zetaTokenConsumerUniV3', networkName as any)
...
🔗 Setting interactors for a contract on ${source}`)
	...
	for (const counterparty in contracts) {
		...
		await (await contract.setInteractorByChainId(chainId, counterpartyContract)).wait()
		console.log(
...
task('deploy', 'Deploy the contract')
	.addParam(
		'networks',
		`Comma separated list of networks to deploy to (e.g. ${getSupportedNetworks('ccm')})`
	)
	.setAction(main)
